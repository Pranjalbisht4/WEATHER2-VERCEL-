...
const Map = () => {
  ...
  const fetchLayerData = async (layer) => {
    try {
      ...
      const response = await fetch(`${baseurl}/api/weather/coords?lat=51.5074&lon=-0.1278`);
  ...
  const handleMapClick = async (lat, lng) => {
    try {
      ...
      const weatherResponse = await fetch(`${baseurl}/api/weather/coords?lat=${lat}&lon=${lng}`);
      const weatherResult = await weatherResponse.json();
      setWeatherData(weatherResult);

      // Fetch marine data for clicked location
      const marineResponse = await fetch(`${baseurl}/api/marine?lat=${lat}&lon=${lng}`);
...
☁️ Cloud Cover: ${weatherResult.current?.clouds ?? weatherResult.clouds?.all ?? "N/A"}%
  ...
  const handleExport = async () => {
    try {
      ...
      const url = `${baseurl}/api/map/export?title=${encodeURIComponent(title)}${coords}`;
